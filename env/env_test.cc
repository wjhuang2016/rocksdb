...
TEST_P(EnvPosixTestWithParam, PosixRandomRWFile) {
...
#ifdef OS_LINUX
  // Turn the file to posix and test it
  auto posix_file_ptr = dynamic_cast<PosixRandomRWFile*>(file.release());
  ASSERT_NE(posix_file_ptr, nullptr);
  std::unique_ptr<PosixRandomRWFile> posix_file;
  posix_file.reset(posix_file_ptr);

  uint64_t file_size = 0;
  ASSERT_OK(posix_file->GetSizeOnDisk(&file_size));
  ASSERT_EQ(file_size, 4096);

  // Write many data for punching hole
  char write_buf[4096];
  for (int32_t i = 0; i < 10; ++i) {
    write_buf[0] = i;
    ASSERT_OK(posix_file->Write(4096 * i, Slice(write_buf, 4096)));
  }

  ASSERT_OK(posix_file->GetSizeOnDisk(&file_size));
  ASSERT_EQ(file_size, 40960);

  // Punch hole
  for (int32_t i = 0; i < 10; ++i) {
    if (i % 2 == 0) {
      ASSERT_OK(posix_file->PunchHole(4096 * i, 4096));
    }
  }
  ...
  // Make sure we skip the hole
  uint64_t offset = 0;
  for (int32_t i = 0; i < 10; ++i) {
    offset = 4096 * i;
    ASSERT_OK(posix_file->SeekNextData(&offset));
    if (i % 2 == 0) {
      ASSERT_EQ(offset, 4096 * (i + 1));
    } else {
      ASSERT_EQ(offset, 4096 * i);
    };
  }

  auto file_ptr = dynamic_cast<RandomRWFile*>(posix_file.release());
